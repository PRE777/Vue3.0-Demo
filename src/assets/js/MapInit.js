 ...
 */
export function defaultInitCesium(
  id,
  mapType,
  isImage,
  sceneModeType = "3D",
  options = null,
  completionByDefaultOptions
) {
  completionByDefaultOptions = !!completionByDefaultOptions;

  var imageryProvider;
  if (mapType == "ArcGIS") {
    imageryProvider = isImage
      ? getArcGISImageProvider()
      : getArcGISStreetProvider();
  } else if (mapType == "tianDitu") {
    imageryProvider = isImage
      ? getTianDiTuImageProvider()
      : getTianDiTuStreetProvider();
  } else if (mapType == "Amap") {
    // 高德
    imageryProvider = isImage
      ? getAmapImageProvider()
      : getAmapStreetProvider();
  } else if (mapType == "baidu") {
    imageryProvider = getBaiDuProvider();
  } else if (mapType == "google") {
    imageryProvider = isImage
      ? getGoogleImageProvider()
      : getGoogleStreetProvider();
  } else if (mapType == "wms") {
...
function getTianDiTuImageProvider() {
  return new Cesium.WebMapTileServiceImageryProvider({
    url:
      "http://t0.tianditu.com/img_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=img&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default&format=tiles&tk=44964a97c8c44e4d04efdf3cba594467",
    layer: "tdtBasicLayer",
    style: "default",
    format: "image/jpeg",
...
function getTianDiTuStreetProvider() {
  return new Cesium.WebMapTileServiceImageryProvider({
    url:
      "http://t0.tianditu.com/vec_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=vec&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default&format=tiles&&tk=42f7b634383f7e34d0ef339b26252b02",
...
function getGoogleStreetProvider() {
  return new Cesium.UrlTemplateImageryProvider({
    url:
      "http://mt2.google.cn/vt/lyrs=m&hl=zh-CN&gl=CN&src=app&x={x}&y={y}&z={z}&s=G",
  });
}
  ...
  switch (mapType) {
    ...
    case "tianDitu":
      imageryProvider = new Cesium.WebMapTileServiceImageryProvider({
        url:
          "http://t0.tianditu.com/cva_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=cva&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default&format=tiles&tk=be6dab35ca077d539685f353e95bd02b",
        layer: "tdtAnnoLayer",
        style: "default",
        format: "image/png",
        tileMatrixSetID: "GoogleMapsCompatible",
