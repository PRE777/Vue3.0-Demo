...
export default class DrawstraightArrow {
  constructor(viewer) {
    this.viewer = viewer;
  ...
  loadStraightArrow(data) {
    ...
    let firstPoint = $this.cartesianToLatlng(p1);
    let endPoints = $this.cartesianToLatlng(p2);
    let arrow = [];
    let res = $this.fineArrow(
    ...
    let arrowEntity = $this.viewer.entities.add({
      polygon: {
  ...
  startCreate() {
    ...
    this.handler.setInputAction(function(evt) {
      ...
      $this._positions = [];
      $this.floatingPoint.position.setValue(cartesian);
      let straightArrow = $this.loadStraightArrow($this._straightArrowData);
      //加载
    ...
    let arrowEntity = $this.viewer.entities.add({
      polygon: {
        hierarchy: new Cesium.CallbackProperty(function() {
          ...
          let p1 = $this._positions[0];
          let p2 = $this._positions[length - 1];
          let firstPoint = $this.cartesianToLatlng(p1);
          let endPoints = $this.cartesianToLatlng(p2);
          let arrow = [];
          let res = $this.fineArrow(
  ...
  cartesianToLatlng(cartesian) {
    let latlng = this.viewer.scene.globe.ellipsoid.cartesianToCartographic(
      cartesian
    );
  ...
  wholeDistance(t) {
    let o = 0;
    for (let e = 0; e < t.length - 1; e++) o += this.distance(t[e], t[e + 1]);
    return o;
  }
  distance(t, o) {
